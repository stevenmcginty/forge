...
| 7 | The desktop's voice stack — the Parakeet sidecar, the wake word, the voice agent — the screenshot tray, the overlay, Forge Mobile and Forge TV stay out. **Dictation is in, but it is the browser's own**: the composer's microphone (web/src/lib/speech.ts) is the Web Speech API, so what you say becomes words in the box with nothing crossing the tunnel a typed sentence would not; Auto-send makes a pause the Send for somebody with no keyboard at all. Firefox has no recogniser and the button says so. **The desktop's screen does not** stay out either: a browser may watch it, and behind a second lock drive it. | A browser tab has no microphone worth plumbing back to the PC, and it turned out not to need one — Chrome, Edge and Safari each carry a recogniser of their own. The screen is the one hardware-bound thing that earns its risk — seeing and fixing the machine you are away from is most of what "Forge from anywhere" means, and a terminal cannot answer a dialog that is not in one. It travels over the existing WebSocket rather than WebRTC, whose media is peer-to-peer over UDP and would need a paid TURN relay to cross this tunnel at all. |
