...
| 5 | The browser **mirrors** the desktop's sessions. It does not spawn its own. | One workspace, one truth. Open a tab in the browser and it appears on the desk. Forge Mobile already works this way. **Where that truth lives moved:** the split tree was the renderer's until a dead window stranded a phone three hundred miles away, and the main process owns it now (`electron/layout-engine.ts`), with the renderer following on `IPC.workspaceReplaced`. Still one workspace, still one truth — and a browser no longer waits on a window to get it. |
...
| 7 | Voice/dictation, the screenshot tray, the overlay, Forge Mobile and Forge TV stay out. **The desktop's screen does not**: a browser may watch it, and behind a second lock drive it. | A browser tab's microphone only records: the composer's mic button ships the recording down the socket as `dictate` chunks and the desktop's speech-to-text key (Groq, else Gemini) turns it into words, which the browser then types into the pane. The CLIs' own `/voice` cannot help — it hears the desktop's microphone, and the phone is miles from it. **A keyboard is served by the same reasoning turned around**: the ←↑↓→ / Tab / Esc row is a thumb's interface, so on a keyboard those keys work as themselves whenever the box is empty (web/src/components/Composer.tsx, `RAW_KEYS`; scripts/web-keys-check.mjs) — the same empty-box rule that already makes Enter the Enter key rather than a Send, and answering a menu from a laptop is arrows and Enter rather than six trips to the mouse. A draft hands every one of them straight back to the textarea, because a composer that cannot be edited with the arrow keys is a worse thing than a menu that needs the mouse. The screen is the one hardware-bound thing that earns its risk — seeing and fixing the machine you are away from is most of what "Forge from anywhere" means, and a terminal cannot answer a dialog that is not in one. It travels over the existing WebSocket rather than WebRTC, whose media is peer-to-peer over UDP and would need a paid TURN relay to cross this tunnel at all. |
...
- No browser-side voice, screenshots, Mobile or TV in the browser. The desktop's screen *is*
