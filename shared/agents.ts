...
const MODEL_FAMILIES: Record<string, AgentModelSpec[]> = {
  ...
  agy: [
    { id: 'gemini-3.8-flash-high', label: 'Gemini 3.8 Flash (High)', note: 'newest Flash, deepest effort' },
    { id: 'gemini-3.8-flash-medium', label: 'Gemini 3.8 Flash (Medium)', note: 'newest Flash, balanced' },
    { id: 'gemini-3.8-flash-low', label: 'Gemini 3.8 Flash (Low)', note: 'newest Flash, fastest' },
    { id: 'gemini-3.7-flash-high', label: 'Gemini 3.7 Flash (High)', note: 'previous Flash, deepest effort' },
    { id: 'gemini-3.7-flash-medium', label: 'Gemini 3.7 Flash (Medium)', note: 'previous Flash, balanced' },
    { id: 'gemini-3.7-flash-low', label: 'Gemini 3.7 Flash (Low)', note: 'previous Flash, fastest' },
    { id: 'gemini-3.6-flash-high', label: 'Gemini 3.6 Flash (High)', note: 'older Flash, deepest effort' },
    { id: 'gemini-3.6-flash-medium', label: 'Gemini 3.6 Flash (Medium)', note: 'older Flash, balanced' },
    { id: 'gemini-3.6-flash-low', label: 'Gemini 3.6 Flash (Low)', note: 'older Flash, fastest' },
  ...
  gemini: [
    ...
    { id: 'gemini-3.6-flash', label: 'Gemini 3.6 Flash', note: 'older Flash' },
  ...
  opencode: [
    ...
    { id: 'google/gemini-3.8-flash', label: 'Gemini 3.8 Flash', note: 'Google, newest Flash' },
    { id: 'google/gemini-3.7-flash', label: 'Gemini 3.7 Flash', note: 'Google' },
...
/**
 * How *this* pane's CLI takes dictation from the keyboard, so a mic button on
 * a phone can drive the CLI's own microphone rather than a browser's.
 *
 * The button never records anything itself. It types the keystrokes the TUI
 * documents for its built-in dictation, and the CLI listens on the desktop's
 * microphone and puts the words in its own prompt — the same words, in the
 * same box, as pressing the key at the desk. That is why nothing here mentions
 * audio: over a PTY there is nothing but bytes to send.
 */
export interface DictationDialect {
  /**
   * A slash command to type once per pane before the first recording, or
   * null when the CLI needs no set-up. Sent as words, then Enter a beat
   * later, like every other slash command Forge types.
   */
  arm: string | null
  /** How long the TUI needs after `arm` before `start` will be honoured. */
  armSettleMs: number
  /** The keystroke that starts a recording. */
  start: string
  /** The keystroke that stops it. */
  stop: string
  /** True when stopping also submits the transcript as the prompt. */
  submits: boolean
}

/**
 *  - **Claude Code** (every `claude`-exe profile, including GLM): `/voice`
 *    is push-to-talk, and its default `hold` mode reads key-repeat, which a
 *    single byte down a PTY cannot produce. `/voice tap` switches the (global,
 *    persisted) setting to tap mode: Space on an empty prompt starts a
 *    recording, Space again stops it and submits. Verified against Claude
 *    Code 2.1.263 over a PTY (2026-09-06): Space arrived, `REC · tap to send`
 *    showed within 400 ms, the second Space submitted the transcript.
 *  - **Antigravity** (`agy` 1.1.21+): `/voice` or F5 starts and stops
 *    dictation, which lands in the prompt without submitting. F5 is the
 *    keystroke, because `/voice` typed as words would need Enter and its own
 *    settle each way. (Changelog, 1.1.21.)
 *  - **Codex, Grok, Kimi, OpenCode, Qwen, a shell**: no dictation Forge can
 *    reach — null, and the caller falls back to the browser's own.
 */
export function dictationDialect(command: string): DictationDialect | null {
  const exe = commandExe(command)
  if (exe === 'claude') {
    return { arm: '/voice tap', armSettleMs: 2500, start: ' ', stop: ' ', submits: true }
  }
  if (exe === 'agy' || exe === 'antigravity') {
    return { arm: null, armSettleMs: 0, start: '\x1b[15~', stop: '\x1b[15~', submits: false }
  }
  return null
}
