/**
 * The hub's four voice tools — navigate by a terminal's name, list the
 * terminals by name, run a saved prompt, put a file on the Board — in words and
...
export const HUB_TOOL_SPECS: HubToolSpec[] = [
  {
    ...
    description: [
      'Go to a pane, the Wall or the Board, the way Steve says it: a terminal’s name ("Zeb", "Viggo"), an agent ("the codex one"), "the wall" (every terminal at once) or "the board" (agent images and artifacts).',
      'Every terminal has one name, the one on its tab — list_panes_with_names shows them. Brings the pane’s tab forward and puts the keyboard in it. If more than one pane matches, the answer lists them: ask which one, never guess. If he says "canvas", do not pick: ask "the Wall or the Board?".'
    ].join(' '),
    parameters: {
      type: 'object',
      properties: { name: { type: 'string', description: 'The target in his words: "Zeb", "the codex one", "the wall", "the board"' } },
      required: ['name']
    }
  },
  {
    ...
    description:
      'List every terminal in the open project by its name, with its agent, and which one is focused. Use it before navigating by name when you are not sure what exists.',
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        pane: { type: 'string', description: 'Optional: which pane, by its name ("Zeb") or in words. Default: the focused one' },
